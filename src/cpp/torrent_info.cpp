...
extern "C" {
  ...
  boost::intrusive_ptr<torrent_info>* new_torrent_info_filename(char const* filename, int flags = 0) {
    ...
    boost::intrusive_ptr<torrent_info>* ti_ptr = (boost::intrusive_ptr<torrent_info>*)new torrent_info(filename_str, ec, flags);
    if (ec) {
  ...
  boost::intrusive_ptr<torrent_info>* new_torrent_info_torrent_buffer(std::vector<char>* torrentbuffer, int length, int flags = 0) {
    ...
    libtorrent::error_code ec;
    boost::intrusive_ptr<torrent_info>* ti_ptr = (boost::intrusive_ptr<torrent_info>*)new torrent_info(&v[0], v.size(), ec, flags);
    if (ec) {
      std::cout << ec.message() << std::endl;
    }
    return ti_ptr;
